...
namespace Inkluzitron.Extensions
{
    static public class TimeSpanExtensions
    {
        static public string FullTextFormat(this TimeSpan timeSpan)
        {
            ...
            {
                if (timeSpan.Seconds == 0 || timeSpan.Seconds > 4) parts.Add($"{timeSpan.Seconds} sekund");
                else if (timeSpan.Seconds == 1) parts.Add("1 sekundu");
                else parts.Add($"{timeSpan.Seconds} sekundy");
            }

            if (parts.Count == 0) return "0 sekund";
